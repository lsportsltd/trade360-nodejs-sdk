...
describe('BaseBet Entity', () => {
  ...
  describe('JSON Serialization', () => {
    it('should serialize BaseBet with BigInt id using toJSON method', () => {
      const plain = {
      ...
      const baseBet = plainToInstance(BaseBet, plain, {
        excludeExtraneousValues: true,
      });

      const jsonResult = baseBet.toJSON();

      expect(jsonResult.Id).toBe('123456789012345678901234567890');
      expect(jsonResult.Name).toBe('Test Bet');
      expect(jsonResult.Price).toBe('1.50');
      expect(jsonResult.Status).toBe(BetStatus.Open);
      expect(jsonResult.IsChanged).toBe(1);
    });

    it('should handle JSON.stringify with toJSON method', () => {
      const plain = {
        Id: '123456789012345678901234567890',
        Name: 'Test Bet',
      ...
      const jsonString = JSON.stringify(baseBet);
      const parsed = JSON.parse(jsonString);

      expect(parsed.Id).toBe('123456789012345678901234567890');
      expect(parsed.Name).toBe('Test Bet');
      expect(parsed.LastUpdate).toBe('2024-01-01T00:00:00.000Z');
    });

    it('should handle JSON.stringify with arrays of BaseBet objects', () => {
      const plainBets = [
        { Id: '111', Name: 'Bet 1' },
        { Id: '222', Name: 'Bet 2' },
        { Id: '333', Name: 'Bet 3' },
      ...
      const baseBets = plainBets.map(plain => 
        plainToInstance(BaseBet, plain, { excludeExtraneousValues: true })
      );

      // This should not throw "Do not know how to serialize a BigInt" error
      const jsonString = JSON.stringify(baseBets);
      const parsed = JSON.parse(jsonString);

      expect(parsed).toHaveLength(3);
      expect(parsed[0].Id).toBe('111');
      expect(parsed[1].Id).toBe('222');
      expect(parsed[2].Id).toBe('333');
    });
  });
});
